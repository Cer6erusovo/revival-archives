...
- 第一章已经锁定旧教室区域的第一轮鬼锁猎杀、厕所隔间提示，以及物理越界、拖救、关节迟缓、单实体处决、永久破口、杀人增强、同时覆盖、人数优先和落锁预兆；驾驭和其后剧情仍未锁定。
...
- 力量增强后，鬼锁一轮可以同时封锁多个有人空间，但始终只有一个实体，并在覆盖范围中央突然出现。显现后它实际向目标移动，声音方向可被追踪；它优先处决人数最少的空间，玩家可以用已有破口进行高风险单人诱鬼。
- 落锁前，门把手、锁舌金属声和松动玻璃会出现短促变化。声音从覆盖边缘向中央收拢，用于提示大致范围和实体出现方向，但不能被打断来取消落锁。
...
鬼锁的识别、主要目标优先级、物理越界救援、防御和失败反馈已经锁定到足以进入窄范围第一人称机制验证。下一项仍需用户参与的设计决策是：相同人数空间之间怎样决定第二优先级，以及主角怎样主动驾驭鬼锁。未确认前不得直接制作 Godot 关卡，也不得补写白月光失踪机制或第一章结局。
