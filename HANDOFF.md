...
- 第一章已经锁定旧教室区域的第一轮鬼锁猎杀、厕所隔间提示，以及物理越界、拖救、关节迟缓、实体处决、永久破口和杀人增强；驾驭和其后剧情仍未锁定。
...
- 鬼锁具有扩张的覆盖区域和当前猎杀场所。覆盖区域内所有人的关节会逐渐迟缓直至完全不能活动；完全锁死不是死亡，鬼锁必须实体接触后才会立即掰断头颈。
- 越过当前房间或隔间的物理边界可以躲过该次处决；若外侧仍被覆盖，迟缓继续累积。只有离开最外层覆盖边界才停止恶化并慢慢恢复。
- 死人会让锁死、处决转向、覆盖扩张和完整门窗抗性加快。已有破口不会复原；破坏玻璃只创造逃生路线，不会令厉鬼停手。
- 厕所隔间完成第一次成功拖救。后续以预留破口、外部接应、清理路线、搀扶 / 拖拽和救援者轮换推进，不再依靠连续杀死主角队伍或白月光队伍。
...
鬼锁的识别、物理越界救援、防御和失败反馈已经锁定到足以进入窄范围第一人称机制验证。下一项仍需用户参与的设计决策是：鬼锁选择下一个空间的精确条件与转向优先级，以及主角怎样主动驾驭鬼锁。未确认前不得直接制作 Godot 关卡，也不得补写白月光失踪机制或第一章结局。
